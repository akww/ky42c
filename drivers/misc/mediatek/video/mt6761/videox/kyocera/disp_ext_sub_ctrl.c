...
int disp_ext_sub_blank_ctrl(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_POWERDOWN:
		next_status = DISP_SUB_STATE_PWR_OFF;
		break;
#ifdef CONFIG_DISP_EXT_SUB_ALWAYS_ON
	case FB_BLANK_ALLWAYSON:
		next_status = DISP_SUB_STATE_ALLWAYSON;
		break;
...
int disp_ext_sub_device_init( struct platform_device *pdev )
{
	...
	ret = disp_ext_sub_panel_pinctrl_init(pdev, pdata);
	if(ret){
		pr_err("%s end - fail pinctrl_init\n",__func__);
		return ret;
	}
	...
	ret = disp_ext_sub_subdispinfo_init(pdata);
	if(ret){
		pr_err("%s end - fail subdispinfo_init\n",__func__);
		return ret;
	}
...
int disp_ext_sub_get_dt( struct platform_device *pdev )
{
    ...
    //detect device from devicetree
	ret = disp_ext_sub_get_panel_detect( pdev->dev.of_node, pdata );
	if(ret){
		pr_err("%s end - fail get panel detect\n",__func__);
	...
	ret = disp_ext_sub_get_spi_dt(pdev->dev.of_node, &pdev->dev, &pdata->spi_data);
	if(ret){
		pr_err("%s end - fail spi dt\n",__func__);
		disp_ext_sub_pdata_dump( pdata );
		return ret;
	}
...
#ifdef KCDISP_CUST
	fbi->kdisp_disp_type = KDISP_DISP_TYPE_SUBDISP;
#endif /* KCDISP_CUST */
